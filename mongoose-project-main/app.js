...
let praise = new Person({
  name:'praise',
  age: 25,
  gender: 'female',
  favouriteFoods: ['Rice', 'sharwama', 'pizza']
})
...
async function updateHumanId() {
  try{
    ...
    .findOneAndUpdate({name: 'praise'}, {age: 45}, { new: true })
    .then( (data) => {
      console.log(data)
    })
  }catch (error){
...
async function removeMany(){
  try{
    ...
    .deleteMany({name : 'tolu'}) // using .deleteMany because Node says .remove is not a fucntion, it is deprecated.
  }catch(error){
    console.log(error.message)
  }
}
